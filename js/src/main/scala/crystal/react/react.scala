...
import japgolly.scalajs.react.Reusability
  ...
  implicit class StreamOps[F[_], A](private val s: fs2.Stream[F, A]) {
    ...
    def render(implicit ce: ConcurrentEffect[F]): ReactStreamRendererComponent[A] = 
  ...
  object implicits {
    implicit class CallbackToOps[A](private val self: CallbackTo[A]) {
      ...
      @inline def toStream[F[_] : Sync]: fs2.Stream[F, A] = fs2.Stream.eval(self.to[F])
    }
    ...
    implicit class EffectAOps[F[_], A](private val self: F[A]) extends AnyVal {
      def startAsCallback(cb: Either[Throwable, A] => IO[Unit])(implicit effect: Effect[F]): Callback =
        CallbackTo.lift(() =>
          Effect[F].runAsync(self)(cb).unsafeRunSync()
        )

      def startAsCallbackAndThen(cb: A => Callback)(implicit effect: Effect[F]): Callback =
        startAsCallback {
    ...
    implicit def viewCtxReusability[F[_], C, A](implicit r: Reusability[A]): Reusability[ViewCtx[F, C, A]] =
